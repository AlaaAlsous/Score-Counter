...
public static class MatchEndpoints
{
    public static void MapMatchEndpoints(this WebApplication app)
    {
        ...
        app.MapPost("/api/match/{id}/player", (string id, string playerName, MatchStore store) =>
        {
            ...
            if (store.TryGetMatch(id, out var match))
            {
                lock (match!)
                {
                    ...
                    if (match.Players.Count >= match.MaxPlayers) return Results.BadRequest("Matchen är full.");
        ...
        app.MapPut("/api/match/{id}/player/{playerId}/score", (string id, string playerId, int? newScore, int? amount, string? operation, MatchStore store) =>
        {
            ...
            if (store.TryGetMatch(id, out var match))
            {
                lock (match!)
                {
                    ...
                    if (player == null) return Results.NotFound("Spelaren hittades inte i matchen.");
                    if (newScore is { } score) player.Score = score;
                    else if (amount.HasValue && !string.IsNullOrWhiteSpace(operation))
                        player.Score += operation == "decrease" ? -amount.Value : amount.Value;
                    else return Results.BadRequest("Antingen newScore eller amount+operation krävs.");
                    store.UpdateMatch(id, match);
        ...
        app.MapPost("/api/match/{id}/finish", (string id, MatchStore store) =>
        {
            ...
            if (store.TryGetMatch(id, out var match))
            {
                lock (match!) { match.IsFinished = true; store.UpdateMatch(id, match); }
                return Results.Ok(match);
            }
            return Results.NotFound("Matchen hittades inte.");
        });
